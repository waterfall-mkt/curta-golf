...
/// @notice A custom implementation of the ERC-721 standard to allow Curta Golf
/// to forcibly transfer tokens. It will only be used when a course receives a
/// new King (i.e. lowers the previous leading solution's gas usage).
/// Additionally, this contract efficiently keeps track of the last 28 bits
/// (LSb) of the King's address by packing it into a single 32-byte slot with
/// the token owners.
abstract contract KingERC721 {
    // -------------------------------------------------------------------------
    // Events
    // -------------------------------------------------------------------------
    ...
    function _forceTransfer(address _to, uint256 _id) internal {
        ...
        address from = _tokenData[_id].owner;
