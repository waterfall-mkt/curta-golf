...
interface ICourse {
    ...
    /// the score).
    /// @dev `_solution` will always be deployed by the `CurtaGolf` contract
    /// from player bytecode submissions. Similarly, `_seed` is used to generate
    /// the inputs, and it will also always be passed in by the `CurtaGolf`
    /// contract.
    /// @param _solution The address of contract with the solution to run.
    /// @param _seed The seed used to generate the inputs for the solution.
    /// @return The gas used by the solution.
    function run(address _solution, uint256 _seed) external view returns (uint32);
}
